...
# ---------------------------------------------------------------------------
# 5) 入库源解析：ep targetId 回退与缺 file_path 的显式错误
# ---------------------------------------------------------------------------

def test_require_ingest_source_ep_shape(tmp_path, monkeypatch):
    ...
    document_id, file_path = routes_module._require_ingest_source(request)
...
# ---------------------------------------------------------------------------
# 6) 联调契约样本 fixture 存在且 JSON 合法（帮助 ep 侧联调）
# ---------------------------------------------------------------------------
