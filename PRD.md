...
- [ ] `web`: 将右侧栏接入真实当前线程目录、近期证据、笔记和结构化输出数据；现有导航壳已完成，但还需要和 API / RAG 同步成真实可用能力。
- [ ] `web`: 将统一文档预览接入真实 PDF、HTML、代码块预览；现有预览壳已完成，但还需要和 API / RAG 的来源格式、文件流、清洗规则同步。
- [ ] `web`: 将 AI 整理入口接入真实思维导图/流程图生成结果；现有入口只是本地占位，还需要和 RAG 的结构化中间表示与后端契约同步。
