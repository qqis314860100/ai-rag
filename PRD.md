...
- 聊天主流程仍需做一次完整端到端回归，尤其是：真实 RAG 流式生成、中断、重试、编辑、删除确认、收藏/反馈、引用详情、长会话滚动恢复。
- 右侧栏已改造成“当前会话导航器”壳，包含当前线程目录、近期证据、会话笔记占位和结构化输出入口；统一文档预览壳已支持文本、Markdown、原文切换，PDF、HTML、代码块仍是禁用占位；真实图谱生成和笔记持久化仍处于待实现状态。
- API/RAG 侧仍需补齐消息分支截断测试、source detail 契约、文档预览契约和检索 metadata 增强。
...
- [ ] `web`: 做一次聊天端到端回归，覆盖登录、发送、中断、切换会话、滚动恢复、引用详情、复制、删除确认、点赞/点踩、收藏、重试，确认 UX/UI 不闪跳。
...
1. 先做 `web` AI 整理入口占位，确保“思维导图/流程图待接入”只走本地提示，不调用不存在的后端 API。
2. 再做 `web` 聊天端到端回归，确认发送、中断、重试、编辑、删除、滚动恢复、引用详情、收藏/反馈都无闪跳。
3. 然后做 `api` 消息删除/编辑语义加固，保证前端动作背后行为可靠。
4. 最后做 `api` source detail 契约和 `rag` metadata 扩展，让引用详情和多格式预览有真实数据支撑。
