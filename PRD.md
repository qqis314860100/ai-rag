...
- [ ] `web`: 验证问答卡片动作语义。用户问题 hover 显示复制、编辑、删除；删除必须二次确认，并连带删除对应回答或后续分支。回答不提供删除，提供复制、点赞、点踩、收藏、重试这些功能是否完善。
- [ ] `web`: 输入框发送按钮在生成已经中切换为中断按钮，需验证中断后不会生成可收藏/可反馈的临时消息。
...
- [ ] `web`: 做一次聊天端到端回归，覆盖登录、发送、中断、切换会话、滚动恢复、引用详情、复制、删除确认、点赞/点踩、收藏、重试，ux,ui。
...
1. 先检查 `web` 滚动与问答卡片动作是否流畅，因为这是当前用户可见的。
2. 再完成 `web` 右侧会话导航壳，把引用、章节、笔记和结构化输出入口放到正确位置，需考虑内容长度是否放的下，如果引用内容过长会导致左侧过长，优先考虑ui设计。
