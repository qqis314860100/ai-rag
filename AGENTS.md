...
- 第 1 层：`AGENTS.md`，通用 agent 入口和项目短记忆
- 第 2 层：`skills/`，可复用知识和场景工作流
- 第 3 层：`hooks/`，风险命令拦截、前后置检查和审计记录
- 第 4 层：`subagents/`，隔离上下文、并行执行和结果交接
- 第 5 层：`plugins/`，团队能力分发和版本同步

## 先读这里

日常改动优先读最少上下文：

- 不熟悉项目：先读 [README.md](README.md)
...
- 本文件和 [CLAUDE.md](CLAUDE.md) 只是短入口摘要；如果和执行规则有偏差，以执行规则为准。
...
- 代码输出默认追求整洁、可复用、可维护、低耦合和易用；新增抽象必须服务真实复杂度，不为了拆而拆。
- 新增代码注释优先使用中文，只在业务规则、边界条件或非显然算法处添加。
